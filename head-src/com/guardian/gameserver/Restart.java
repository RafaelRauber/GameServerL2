...
public class Restart
{
      ...
      //:D
        ...
        {
				_log.info("[Restart System]: Sistema de Restart Automatico Ativado");
                try
                {
                        ...
                        _log.info("[Auto-Restart]: Proximo restart em " + NextRestart.getTime().toString());
            ThreadPoolManager.getInstance().scheduleGeneral(new StartRestartTask(), flush2);
                }
                catch (Exception e)
                {
                        System.out.println("[Auto-Restart]: O Restart automatico deu um erro nas configuracoes !");
                }
        }
        ...
        class StartRestartTask implements Runnable
        {
                public void run()
                {
           _log.info("[GameServer] Restart Automatico Iniciado.");
